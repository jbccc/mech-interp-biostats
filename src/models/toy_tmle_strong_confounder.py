...
class ToyTMLEStrongConfounder(Model):
    """Trainer for the Toy TMLE model with Strong Confounders."""

    def __init__(self) -> None:
        """Initialize the ToyTMLEStrongConfounder model."""
        super().__init__()
        self.seed = 42
        ...
        self.model_dir = "./checkpoints/toy_tmle/strong_confounder"
    ...
    def model(self) -> ToyTMLE:
        """Returns the TMLE Strong Confounder model with hardcoded parameters.

        Returns:
            torch.nn.Module: The TMLE Strong Confounder model.
        """
    ...
    def get_dataloaders(self) -> DataLoaders:
        ...
        if self.dataloaders is None:
            print("Loading and preparing data (Strong Confounder)...")
            data, feature_cols = generate_synthetic_data(
    ...
    def _train_step(
        ...
        Returns:
            dict: Dictionary containing loss values for the batch.
        """
    ...
    def save_loss(self, losses) -> dict[str, float]:
    ...
    def get_plots(self) -> list[Figure]:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Training and Validation Losses (Strong Confounder)")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        figures.append(fig)
